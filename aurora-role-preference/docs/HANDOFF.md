...
| 15 | `Name isn't valid` on a column that exists, is the right type, and works in another app against the SAME table | the Data pane shows `<table>_1`. Power Apps appends `_1` when a data source name collides, so the app holds **two bindings to one table** — and the formulas reference the older one, whose cached schema predates the column | remove **both** entries and re-add the table once; the survivor keeps its `_1` name if you only delete the stale one |
